...
import { UserStateEnum } from 'src/user/enum/UserStateEnum';
...
export class FillUsers1690577318575 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    users.forEach((user) => {
      queryRunner.query(
        `INSERT INTO user (roleId, email, isEmailConfirmed, password, salt, id, state)
            SELECT id, '${user.email}', '${
          user.isEmailConfirmed
        }', '${password}', '${salt}','${uuidv4()}', '${user.state}'
            FROM role
